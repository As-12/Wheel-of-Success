/* Gameshow.JS 
   ...
   Imperative JS Script to handle Game Show App
*/
...
window.addEventListener("DOMContentLoaded", event => {
  ...
  let missed = 0;
  const maxHearts = document.querySelector("#scoreboard").firstElementChild
    .children.length;
  const phrases = [
    "Hello World",
    "Web Development",
  ...
  function checkLetter(buttonClicked) {
    ...
    for (let i = 0; i != listOfLetters.length; i += 1) {
      ...
      if (
        buttonText.toString().toLowerCase() ===
        letter.textContent.toString().toLowerCase()
      ) {
